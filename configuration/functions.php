...
function sitename() {
    require 'configuration/config.php';
    $sql = "SELECT sitename FROM settings LIMIT 1";
    $result = mysqli_query($conn, $sql);

    if(mysqli_num_rows($result) > 0){
        while($siteinfo = mysqli_fetch_assoc($result)){
...
function sitelogo() {
    require 'configuration/config.php';
    $sql = "SELECT sitelogo FROM settings LIMIT 1";
    $result = mysqli_query($conn, $sql);

    if(mysqli_num_rows($result) > 0){
        while($siteinfo = mysqli_fetch_assoc($result)){
            echo $siteinfo['sitelogo'];
        }
    }
}
...
function siteurl() {
    require 'configuration/config.php';
    $sql = "SELECT siteurl FROM settings LIMIT 1";
    $result = mysqli_query($conn, $sql);

    if(mysqli_num_rows($result) > 0){
        while($siteinfo = mysqli_fetch_assoc($result)){
            echo $siteinfo['siteurl'];
        }
    }
}

function bitcoinprice(){
    $url = file_get_contents('https://api.coinbase.com/v2/prices/BTC-USD/spot');
